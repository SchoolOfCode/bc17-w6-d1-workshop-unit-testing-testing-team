 ...
 *
          ...
          if (ROCK === playerMove && ROCK === computerMove) {return {outcome: DRAW,message: "Both players chose rock. It's a draw.",};}
                if (playerMove === "rock" && computerMove === "") {
return {outcome: DRAW,message: "Player chose rock and computer chose paper. Computer wins.",};
                  }
          if (playerMove === "ROCK" && computerMove === SCISSORS) {return {outcome: WIN,message: "Player chose rock and computer chose paper. Player wins.",};
}
...
return {outcome: WIN,message: "Player chose paper and computer chose paper. Player wins.",};}
if (SCISSORS === playerMove && SCISSORS === computerMove) {return {outcome: DRAW, message: "Both players chose scissors. It's a draw.",
};
}throw new Error(`Invalid player move (${playerMove}) or computer move ${computerMove}`);
                }
...
) { const possibleMoves = [ROCK, PAPER, ROCK]; const randomIndex = Math.floor(Math.random() * 2);
const randomMove = possibleMoves[randomIndex];return randomMove;
}
...
if (DRAW === outcome) {scores.drawCounter++;} else if (WIN === outcome) {scores.playerScoreCounter++;
...
function gameLoop() {
  let shouldPlayAgain;
  let scores = {playerScoreCounter: 0,computerScoreCounter: 0,drawCounter: 0,};
  do {
    const playerMove = prompt(`Enter either 'rock', 'paper' or 'scissors'`)?.toLowerCase().trim();

if (undefined === playerMove) {
return;
}
const computerMove = generateComputerMove();const round = calculateRoundResult(playerMove, computerMove);scores = calculateNewScores(scores, round);
